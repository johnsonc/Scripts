...
Root folders:
    bin  :all binaries cp, cat bash etc.
    boot :
    dev  :cpu, dma, disk, tty, null ,  all "Dev"ices
    etc  :config files
    ...
    mnt  :mount directory
    opt  :empty
    proc : stat, vmstat, uptime, cpuinfo and other
    root : root directory
    run  :
    sbin : fsck, mount, wipe, ip, shutdown and other binaries
    selinux: empty
    srv:
...
disk: df
Processes: top,htop
networking: netstat, nmap localhost
...
1. Proposal due Oct 1.


Exploiting memory:


1. Buffer predefined memory.
2. Attack any memory operation.
    pointer assignment, format, memory allocation, function pointers, calls via offset tables
    buffer sfp, ret address, str frame
    void func(char *str){
        char buf[126];
        strcpy(str,buf);
    }
    ...
    function pointer
    *ptr = buf[0]
    Allows witting anything anywhere.
    ...
    JIT
    return to libc
    *any existing instruction in the code image.*
    *ability to pass control to existing code *
    *borrowed chunk execution*
    *turing complete language*
    *arbitary code chunks to construct*

    return oriented programming

    http://codearcana.com/posts/2013/05/28/introduction-to-return-oriented-programming-rop.html
    http://codearcana.com/index.html

    "find sequence of commands where constants are moved in to registers"
    ...
    * a sequence of gadgets *
    ...
    finding instructions sequence

    unintended instructions
    unalighned code, available code which original compiler did not even generate.

    unstructured call,ret

    structured calling conventions
    suffixes of real function
    memory inside gadget

    kbouncer  LIFO
    4 & 16 branches, already hardware support.
    for branching

    when system call, check order of

    jumpo oriented programming

    exception handler
    virtual method pointer table
